...
{
	OverlayManager::AddLog("[system] Extracting 'palettes.tar.gz'...\n");
	...
	if (!decompressObj.OpenArchive(L"BBTAG_IM/Download/palettes.tar.gz"))
	{
		...
		OverlayManager::AddLog("[error] 'BBTAG_IM/Download/palettes.tar.gz'could not be opened: %s\n", pStr);
	...
	for (int i = 0; i < decompressObj.GetCompressedFilesCount(); i++)
	{
		...
		if (fIsFile)
		{
			...
			if (strncmp(((IMPL_t*)pszFileBuffer)->header.filesig, "IMPL", 5) != 0)
			{
				...
				OverlayManager::AddLog("[error] 'palettes.tar.gz' contains non .impl file: '%s'\n", fileName.c_str());
				OverlayManager::DisableLogging();
	...
	OverlayManager::EnableLogging();
	OverlayManager::AddLog("[system] Total of %d palettes had been successfully extracted from 'palettes.tar.gz' (%d succeeded, %d failed)\n", 
...
std::vector<std::wstring> DownloadPaletteFileList(std::wstring wUrl)
{
	...
	OverlayManager::AddLog("[system] Getting online palette list...\n");
	...
	else
	{
		...
		OverlayManager::AddLog("[error] Failed to retrieve online palette list. No data, or 404\n");
	}
...
void DownloadPaletteArchive()
{
	if (!g_interfaces.pPaletteManager->PaletteArchiveDownloaded())
	{
		...
		OverlayManager::AddLog("[system] Downloading latest 'palettes.tar.gz'...\n");
		int res = DownloadUrlBinary(wUrl, (void**)&downlBuf);

		if (res > 0 && downlBuf)
		{
			OverlayManager::AddLog("[system] Finished downloading 'palettes.tar.gz'\n");
			...
			{
				OverlayManager::AddLog("[error] 'BBTAG_IM/Download/palettes.tar.gz'could not be written\n");
