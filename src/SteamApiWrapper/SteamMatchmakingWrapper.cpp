...
SteamAPICall_t SteamMatchmakingWrapper::RequestLobbyList()
{
	...
	AddRequestLobbyListDistanceFilter(k_ELobbyDistanceFilterDefault);
...
{
	LOG(7, "SteamMatchmakingWrapper GetLobbyByIndex %d\n", iLobby);
...
{
	LOG(7, "SteamMatchmakingWrapper CreateLobby %d %d\n", eLobbyType, cMaxMembers);
...
{
	LOG(7, "SteamMatchmakingWrapper GetNumLobbyMembers (0x%p)", steamIDLobby);

	int ret = m_SteamMatchmaking->GetNumLobbyMembers(steamIDLobby);
	LOG(7, " -> %d\n", ret);
...
{
	LOG(7, "SteamMatchmakingWrapper GetLobbyData (0x%p): ", steamIDLobby);
	LOG(7, "%s -> ", pchKey);
	//regionlock disabling
	//if (strcmp(pchKey, "lobbyname") == 0)
	//{
	//	static int num = 0;
	//	static char name[32] = "";
	//	const char* ret = _itoa(num, name, 10);
	//	LOG(7, "returned %s\n", ret);
	//	num++;
	//	return ret;
	//}
	//else if (strcmp(pchKey, "matchtype") == 0)
	//{
	//	const char* ret = "0";
	//	LOG(7, "returned %s\n", ret);
	//	return ret;
...
const char* SteamMatchmakingWrapper::GetLobbyMemberData(CSteamID steamIDLobby, CSteamID steamIDUser, const char *pchKey)
{
	...
	//	pop eax
	//}
	//LOG(7, "\treturn address: 0x%p\n", returnAddress);
	//LOG(7, "\tsteamIDLobby: 0x%p, steamIDUser: 0x%p, pchKey: %s\n", steamIDLobby, steamIDUser, pchKey);
...
{
	LOG(7, "SteamMatchmakingWrapper GetLobbyMemberLimit (0x%p)", steamIDLobby);
	int ret = m_SteamMatchmaking->GetLobbyMemberLimit(steamIDLobby);
	LOG(7, " -> %d\n", ret);
	return ret;
}
...
CSteamID SteamMatchmakingWrapper::GetLobbyOwner(CSteamID steamIDLobby)
{
	...
	//	LOG(7, "SteamLobbyMemberByIndex %d: 0x%p\n", i ,member);
	//}
