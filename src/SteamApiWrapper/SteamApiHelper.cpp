...
void SteamApiHelper::OnUpdateNumberOfCurrentPlayers(NumberOfCurrentPlayers_t *pCallback, bool bIOFailure)
{
	...
	{
		OverlayManager::getInstance().AddLog("[error] Failed to update the current number of ingame players\n");
		current_players = -1;
		return;
	}
	//printf("Number of players currently playing: %d\n", pCallback->m_cPlayers);
#ifdef ENABLE_LOGGING
	OverlayManager::getInstance().AddLog("[debug] Updated the current number of ingame players: %d\n", pCallback->m_cPlayers);
#endif
