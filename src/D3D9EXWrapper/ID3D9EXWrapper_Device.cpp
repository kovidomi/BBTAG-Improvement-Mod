...
HRESULT APIENTRY Direct3DDevice9ExWrapper::Reset(D3DPRESENT_PARAMETERS* pPresentationParameters)
{
	...
	OverlayManager::getInstance().InvalidateDeviceObjects();

	HRESULT ret = m_Direct3DDevice9Ex->Reset(pPresentationParameters);

	OverlayManager::getInstance().CreateDeviceObjects();
...
HRESULT APIENTRY Direct3DDevice9ExWrapper::CreateRenderTarget(UINT Width, UINT Height, D3DFORMAT Format, D3DMULTISAMPLE_TYPE MultiSample, DWORD MultisampleQuality, BOOL Lockable, IDirect3DSurface9** ppSurface, HANDLE* pSharedHandle)
{
	...
	if (Settings::settingsIni.viewportoverride == VIEWPORT_OVERRIDE)
	{
		...
		Settings::savedSettings.origViewport.x = (float)Width;
		Settings::savedSettings.origViewport.y = (float)Height;
		Settings::savedSettings.newViewport.MinZ = 0.0;
		Settings::savedSettings.newViewport.MaxZ = 1.0;

		return m_Direct3DDevice9Ex->CreateRenderTarget(Settings::settingsIni.renderwidth, Settings::settingsIni.renderheight, Format, MultiSample, MultisampleQuality, Lockable, ppSurface, pSharedHandle);
	}

	LOG(7, "SETTINGS\n - viewportoverride %d\n - renderheight %d\n - renderheight %d\n",
		Settings::settingsIni.viewportoverride, Settings::settingsIni.renderheight, Settings::settingsIni.renderwidth);
...
HRESULT APIENTRY Direct3DDevice9ExWrapper::CreateDepthStencilSurface(UINT Width, UINT Height, D3DFORMAT Format, D3DMULTISAMPLE_TYPE MultiSample, DWORD MultisampleQuality, BOOL Discard, IDirect3DSurface9** ppSurface, HANDLE* pSharedHandle)
{
	...
	{
		LOG(3, "\t- modifying to %u %u\n", Settings::settingsIni.renderwidth, Settings::settingsIni.renderheight);
		return m_Direct3DDevice9Ex->CreateDepthStencilSurface(Settings::settingsIni.renderwidth, Settings::settingsIni.renderheight, Format, MultiSample, MultisampleQuality, Discard, ppSurface, pSharedHandle);
	}
...
HRESULT APIENTRY Direct3DDevice9ExWrapper::SetViewport(CONST D3DVIEWPORT9* pViewport)
{
	...
	if (pViewport->Width >= 1280 && Settings::settingsIni.viewportoverride == VIEWPORT_OVERRIDE) //only change the ones above native resolution. there are stuffs like shadows that use 512x512 that must not be changed
	{
...
HRESULT APIENTRY Direct3DDevice9ExWrapper::SetTexture(DWORD Stage, IDirect3DBaseTexture9* pTexture)
{
	...
	//D3D9Utils::DumpTextureToFile(pTexture, D3DXIMAGE_FILEFORMAT::D3DXIFF_BMP, L"C:\\BBTAG_IM\\Textures\\");
...
HRESULT APIENTRY Direct3DDevice9ExWrapper::SetSamplerState(DWORD Sampler, D3DSAMPLERSTATETYPE Type, DWORD Value)
{
	...
	//{
	//	LOG(2, "SetSamplerState Type: %d Value: %d\n", Type, Value);
	//	Value = D3DTEXF_POINT;
	//}
