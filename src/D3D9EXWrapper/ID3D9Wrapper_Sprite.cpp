...
HRESULT ID3DXSpriteWrapper::QueryInterface(REFIID iid, LPVOID *ppv)
{
	...
	HRESULT hRes = m_D3DXSprite->QueryInterface(iid, ppv);
	if (hRes == S_OK)
		*ppv = this;
	else
		*ppv = NULL;
...
ULONG ID3DXSpriteWrapper::Release()
{
	...
	ULONG res = m_D3DXSprite->Release();
	if (res == 0) {
		delete this;
	}
...
HRESULT ID3DXSpriteWrapper::Draw(LPDIRECT3DTEXTURE9 pTexture, CONST RECT *pSrcRect, CONST D3DXVECTOR3 *pCenter, CONST D3DXVECTOR3 *pPosition, D3DCOLOR Color)
{
	...
	//fix for lobby texts, doesnt fix text on the Duel Field Stage
	if (Settings::settingsIni.viewportoverride == VIEWPORT_OVERRIDE)
	{
		D3DXVECTOR2 scale(Settings::savedSettings.newViewport.Width / Settings::savedSettings.origViewport.x, 
						  Settings::savedSettings.newViewport.Height / Settings::savedSettings.origViewport.y);
