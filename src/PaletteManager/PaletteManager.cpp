...
void PaletteManager::LoadPalettesFromFolder()
{
	...
	g_imGuiLogger->AddLog("[system] Loading local custom palettes...\n");
	...
	g_imGuiLogger->AddLog("[system] Finished loading local custom palettes\n");
}
...
void PaletteManager::ApplyDefaultCustomPalette(CharIndex charIndex, CharPaletteHandle & charPalHandle)
{
	...
	{
		g_imGuiLogger->AddLog("[error] Palette file '%s' cannot be set as default: File not found.\n", palName);
		return;
	}
...
void PaletteManager::LoadPalettesIntoVector(CharIndex charIndex, std::wstring& wFolderPath)
{
	...
	if (hFind != INVALID_HANDLE_VALUE) {
		do {
			...
			{
				g_imGuiLogger->AddLog("[error] Unable to open '%s' : not an .impl file\n", fileName.c_str());
				continue;
			}
			...
			if(!utils_ReadFile(fullPath.c_str(), &fileContents, sizeof(IMPL_t), true))
			{
				...
				g_imGuiLogger->AddLog("[error] Unable to open '%s' : %s\n", fileName.c_str(), strerror(errno));
				continue;
			}
			...
			if (strcmp(fileContents.header.filesig, "IMPL") != 0)
			{
				...
				g_imGuiLogger->AddLog("[error] '%s' unrecognized file format!\n", fileName.c_str());
				continue;
			}
			...
			if (fileContents.header.datalen != sizeof(IMPL_data_t))
			{
				...
				g_imGuiLogger->AddLog("[error] '%s' data size mismatch!\n", fileName.c_str());
				continue;
			}
			...
			if (charIndex != fileContents.header.charindex)
			{
				...
				g_imGuiLogger->AddLog("[warning] '%s' belongs to character '%s', but is placed in folder '%s'\n", 
...
void PaletteManager::LoadPaletteSettingsFile()
{
	...
	if (!PathFileExists(wFullPath.c_str()))
	{
		...
		g_imGuiLogger->AddLog("[error] 'palettes.ini' file was not found!\n");
		return;
	}
...
//		g_imGuiLogger->AddLog("[system] 'palettes.ini' already exists\n");
//		return true;
//	}
//
...
//
//	LOG(2, "\tCreated '%s'\n", path.c_str());
//	g_imGuiLogger->AddLog("[system] Created '%s'\n", path.c_str());
//
//	return true;
//}
...
bool PaletteManager::PushImplFileIntoVector(CharIndex charIndex, IMPL_data_t & filledPalData)
{
	...
	{
		g_imGuiLogger->AddLog("[error] Custom palette couldn't be loaded: CharIndex out of bound.\n");
...
void PaletteManager::ReloadAllPalettes()
{
	...
	g_imGuiLogger->AddLogSeparator();
	g_imGuiLogger->AddLog("[system] Reloading custom palettes...\n");
