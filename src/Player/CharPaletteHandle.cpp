...
char * CharPaletteHandler::GetPalFileAddr(char * base, int palIndex, int fileID)
{
...
void CharPaletteHandler::SetPointerPalIndex(int* pPalIndex)
{
	m_pCurPalIndex = pPalIndex;
}

void CharPaletteHandler::SetPointerBasePal(char* pPalBaseAddr)
{
	m_pPalBaseAddr = pPalBaseAddr;
}

void CharPaletteHandler::SetPaletteIndex(int palIndex)
{
...
bool CharPaletteHandler::IsNullPointerPalIndex()
{
	if (m_pCurPalIndex)
		return false;
...
int & CharPaletteHandler::GetPalIndexRef()
{
	return *m_pCurPalIndex;
}

void CharPaletteHandler::ReplaceAllPalFiles(IMPL_data_t *newPaletteData)
{
...
void CharPaletteHandler::ReplaceSinglePalFile(char * newPalData, PaletteFile palFile)
{
...
void CharPaletteHandler::OnMatchInit()
{
	BackupCurrentPal();
...
void CharPaletteHandler::UnlockUpdate()
{
	m_updateLocked = false;
}

int CharPaletteHandler::GetSelectedCustomPalIndex()
{
	return m_selectedCustomPalIndex;
}

void CharPaletteHandler::SetSelectedCustomPalIndex(int index)
{
	m_selectedCustomPalIndex = index;
}

char * CharPaletteHandler::GetOrigPalFileAddr(PaletteFile palFile)
{
	return GetPalFileAddr(m_pPalBaseAddr, m_switchPalIndex1, (int)palFile);
}

const IMPL_data_t & CharPaletteHandler::GetCurrentPalData()
{
...
void CharPaletteHandler::ReplacePalArrayInMemory(char * Dst, const void * Src)
{
...
void CharPaletteHandler::ReplaceAllPalFiles(IMPL_data_t *newPaletteData, int palIndex)
{
...
void CharPaletteHandler::BackupCurrentPal()
{
...
void CharPaletteHandler::UpdatePalette()
{
...
void CharPaletteHandler::LockUpdate()
{
	m_updateLocked = true;
}
