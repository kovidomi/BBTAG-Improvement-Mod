...
// size_arg (for each axis) < 0.0f: align to end, 0.0f: auto, > 0.0f: specified size
//selfmade
void ImGui::ColoredProgressBar(float fraction, const ImVec2& size_arg, const ImVec4 &col, const char* overlay, bool show_percent, bool right_to_left)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return;
	...
	if (right_to_left)
		RenderRectFilledRangeHRightToLeft(window->DrawList, bb, ColorConvertFloat4ToU32(col), 0.0f, fraction, style.FrameRounding);
	else
		RenderRectFilledRangeH(window->DrawList, bb, ColorConvertFloat4ToU32(col), 0.0f, fraction, style.FrameRounding);
...
//Selfmade
bool ImGui::ColorButtonOn32Bit(const char* desc_id, unsigned char val[4], ImGuiColorEditFlags flags, ImVec2 size)
{
...
//selfmade
bool ImGui::ColorEdit4On32Bit(const char* label, unsigned char val[4], ImGuiColorEditFlags flags)
{
...
bool ImGui::ColorPicker4On32Bit(const char* label, unsigned char val[4], ImGuiColorEditFlags flags, const float* ref_col)
{
	...
	if ((flags & ImGuiColorEditFlags_NoInputs) == 0)
	{
		...
		if (flags & ImGuiColorEditFlags_RGB || (flags & ImGuiColorEditFlags__InputsMask) == 0)
			value_changed |= ColorEdit4On32Bit("##rgb", val, sub_flags | ImGuiColorEditFlags_RGB);
		if (flags & ImGuiColorEditFlags_HSV || (flags & ImGuiColorEditFlags__InputsMask) == 0)
			value_changed |= ColorEdit4On32Bit("##hsv", val, sub_flags | ImGuiColorEditFlags_HSV);
		if (flags & ImGuiColorEditFlags_HEX || (flags & ImGuiColorEditFlags__InputsMask) == 0)
			value_changed |= ColorEdit4On32Bit("##hex", val, sub_flags | ImGuiColorEditFlags_HEX);
		PopItemWidth();
	}
