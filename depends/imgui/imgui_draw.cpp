...
void ImGui::RenderRectFilledRangeHRightToLeft(ImDrawList* draw_list, const ImRect& rect, ImU32 col, float x_start_norm, float x_end_norm, float rounding)
{
	if (x_end_norm == x_start_norm)
		return;
	if (x_start_norm > x_end_norm)
	...
	ImRect temp_rect(rect);

	float tempx = temp_rect.Min.x;
	float tempy = temp_rect.Min.y;
	temp_rect.Min.x = temp_rect.Max.x;
	temp_rect.Min.y = temp_rect.Max.y;
	temp_rect.Max.x = tempx;
	temp_rect.Max.y = tempy;

	ImVec2 p0 = ImVec2(ImLerp(temp_rect.Min.x, temp_rect.Max.x, x_start_norm), temp_rect.Min.y);
	ImVec2 p1 = ImVec2(ImLerp(temp_rect.Min.x, temp_rect.Max.x, x_end_norm), temp_rect.Max.y);
	...
	rounding = ImClamp(ImMin((temp_rect.Max.x - temp_rect.Min.x) * 0.5f, (temp_rect.Max.y - temp_rect.Min.y) * 0.5f) - 1.0f, 0.0f, rounding);
	const float inv_rounding = 1.0f / rounding;
	const float arc0_b = ImAcos01(1.0f - (p0.x - temp_rect.Min.x) * inv_rounding);
	const float arc0_e = ImAcos01(1.0f - (p1.x - temp_rect.Min.x) * inv_rounding);
	const float x0 = ImMax(p0.x, temp_rect.Min.x + rounding);
	if (arc0_b == arc0_e)
	{
		draw_list->PathLineTo(ImVec2(x0, p1.y));
		draw_list->PathLineTo(ImVec2(x0, p0.y));
	...
	if (p1.x > temp_rect.Min.x + rounding)
	{
		const float arc1_b = ImAcos01(1.0f - (temp_rect.Max.x - p1.x) * inv_rounding);
		const float arc1_e = ImAcos01(1.0f - (temp_rect.Max.x - p0.x) * inv_rounding);
		const float x1 = ImMin(p1.x, temp_rect.Max.x - rounding);
		if (arc1_b == arc1_e)
		{
			draw_list->PathLineTo(ImVec2(x1, p0.y));
			draw_list->PathLineTo(ImVec2(x1, p1.y));
